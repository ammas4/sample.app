...
/*
код из Dapper.Contrib с доработками
*/
...
public static partial class SqlMapperExtensions
{
    ...
    static readonly ConcurrentDictionary<RuntimeTypeHandle, IEnumerable<PropertyInfo>> ExplicitKeyProperties = new();
    static readonly ConcurrentDictionary<RuntimeTypeHandle, IEnumerable<PropertyInfo>> TypeProperties = new();
    static readonly ConcurrentDictionary<RuntimeTypeHandle, IEnumerable<PropertyInfo>> ComputedProperties = new();
    static readonly ConcurrentDictionary<RuntimeTypeHandle, string> GetQueries = new();
    static readonly ConcurrentDictionary<RuntimeTypeHandle, string> TypeTableName = new();
    static readonly ConcurrentDictionary<string, string> ColumnName = new();

    static readonly PostgresAdapter AdapterInstance = new();
...
#pragma warning restore CA2211 // Non-constant fields should not be visible
    ...
    public static Func<string, string>? ColumnNameMapper;

    static string GetColumnName(string propertyName)
    {
        if (ColumnName.TryGetValue(propertyName, out string? name)) return name;

        if (ColumnNameMapper != null)
        {
            name = ColumnNameMapper(propertyName);
        }
        else
        {
            name = propertyName;
    ...
    public static long Insert<T>(this IDbConnection connection, T entityToInsert, IDbTransaction transaction = null, int? commandTimeout = null) where T : class
    {
        ...
        for (var i = 0; i < allPropertiesExceptKeyAndComputed.Count; i++)
        {
            ...
            adapter.AppendColumnName(sbColumnList, GetColumnName(property.Name)); //fix for issue #336
    ...
    public static bool Update<T>(this IDbConnection connection, T entityToUpdate, IDbTransaction transaction = null, int? commandTimeout = null) where T : class
    {
        ...
        for (var i = 0; i < nonIdProps.Count; i++)
        {
            ...
            adapter.AppendColumnNameEqualsValue(sb, GetColumnName(property.Name), property.Name); //fix for issue #336
...
#pragma warning restore CA2211 // Non-constant fields should not be visible
    ...
    static PostgresAdapter GetFormatter(IDbConnection connection)
    {
        var name = GetDatabaseType?.Invoke(connection).ToLower()
                   ?? connection.GetType().Name.ToLower();

        if (name != "npgsqlconnection") throw new Exception("Wrong database type");
    ...
    static class ProxyGenerator
    {
        ...
        static AssemblyBuilder GetAsmBuilder(string name)
        {
#if !NET461
        ...
        static void CreateProperty<T>(TypeBuilder typeBuilder, string propertyName, Type propType, MethodInfo setIsDirtyMethod, bool isIdentity)
        {
