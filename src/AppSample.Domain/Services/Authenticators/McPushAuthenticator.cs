...
public interface IMcPushAuthenticator : IAuthStarter
{
    Task ProcessPushAnswer(long msisdn, bool consist);
}

public class McPushAuthenticator : IMcPushAuthenticator
{
    ...
    public async Task<AuthenticatorStartResult> TryStartAsync(AuthSessionBag startBag)
    {
        ...
        if (!userProfile.HasMcApplet) return StartResult(AuthenticatorStartResultType.SimCardNotSupport);
    ...
    {
        var sendResult = await _deviceAdapterRepository.SendPushToMc(msisdn, message);
    ...
    {
        var sendResult = await _userProfileRepository.SendMcPushPin(msisdn, message);
    ...
    {
        Authenticator = AuthenticatorType.PushMc,
    ...
    {
        await _authenticationChainService.TryEndAsync(msisdn.ToString(), AuthenticatorType.PushMc,
